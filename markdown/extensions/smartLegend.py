...
class FigureParser(InFigureParser):
    ...
    def detect(self, element, type):
        ...
        return ((type == "unknown" or type == "Figure") and
                element.tag == "p" and
                ((element.text is not None and self.ree.search(element.text)) or
                 ((element.text is None or element.text.strip() == "") and
                  (len(lelems) == 1 or (len(lelems) == 2 and lelems[0] is element)) and
                  lelems[-1].tag == "img" and
                  (lelems[-1].attrib["src"] not in self.ignoringImg))))
        ...
        if (type == "unknown" or type == "Code") and element.tag == "p":
            ...
            for i in range(hs.html_counter):
                if element.text == hs.get_placeholder(i):
                    ...
                    if (Teste is not None and
                            Teste.tag == "table" and
                            "class" in Teste.attrib and Teste.attrib["class"] == "codehilitetable"):
                        return True
                    else:
                        return False
        return False
...
class VideoParser(InFigureParser):
    def detect(self, element, type):
        if element is None:
            return False
        return (type == "unknown" or type == "Video") and element.tag == "iframe"
...
def makeExtension(configs={}):
    return SmartLegendExtension(configs=configs)
