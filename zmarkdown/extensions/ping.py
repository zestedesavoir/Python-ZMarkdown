...
class PingPattern(Pattern):
    ...
    def handleMatch(self, m):
        ...
        dnode = zmarkdown.util.etree.Element('a')

        dnode.set('class', 'ping')
        dnode.set('href', url)
        dnode.text = zmarkdown.util.AtomicString('@{}'.format(name))
        self.zmarkdown.metadata["ping"].add(name)
        return dnode


class PingExtension(zmarkdown.extensions.Extension):
